...
;	STEP #2
;	YOUR CODE GOES HERE
mov rsi, %1		; sets rsi to address of first char in aSeptLength

%%blanks:					; check for blank spaces loop
	mov r11b, byte [rsi]	; sets r11b to current char in aSeptLength
	cmp r11b, " "			; if char != " "
	jne %%next				; jump to next
	inc rsi					; increment address by 1
	jmp %%blanks			; loop back to blanks
%%next:						; Label to skip loop
	mov r8, 1				; set sign value to positive
	cmp r11b, "-"			; compare sign value char to negative
	jne %%positive			; jump if positive
	mov r8, -1				; set sign value to negative
%%positive:
	inc rsi					; increment address
	mov eax, 0				; set eax to 0
%%charLoop:
	mov r11d, 0				; set r11d register to 0
	mov r11b, byte [rsi]	; get next char

	cmp r11b, NULL			; if char = NULL
	je %%loopExit			; jump to loopExit
	...
	inc rsi					; increment address by 1
	jmp %%charLoop			; jump to start of loop
%%loopExit:					
	imul r8d				; multiply eax by signed value (r8d)
	mov dword [%2], eax		; set length to answer
...
%%loopFour:
	...
	jne %%loopFour				; loop if not equal
...
mov rsi, aSeptLength		; sets rsi to address of first char in aSeptLength
...
loopExit:					
	imul r8d				; multiply eax by signed value (r8d)
	mov dword [length], eax	; set length to answer
; -----
...
loopOne:
	mov eax, dword [diamsArray+rsi*4]		; move current value to eax
	add dword [diamSum], eax				; add value to diamSum

	cmp dword [diamMax], eax
	jge jumpOne								; jump if A >= B
	mov dword [diamMax], eax				; set new max
jumpOne:

	cmp dword [diamMin], eax
	jle jumpTwo								; jump if A <= B
	mov dword [diamMin], eax				; set new min
jumpTwo:

	inc rsi
	loop loopOne							; end of loop

mov eax, dword [diamSum]
cdq
idiv dword [length]							; divide sum by length
mov dword [diamAve], eax					; set average
...
;	YOUR CODE GOES HERE
	...
	mov r8b, "+"				; set sign value to positive
	cmp dword [diamSum], 0		; check if sum is larger than 0
	jge jumpThree				; jump if larger
	mov r8b, "-"				; if less than 0 set sign to negative
jumpThree:

	mov rsi, diamSumString		; get address
	mov r9w, 0					; increment to count digits
	mov r12w, STR_LENGTH-2		; string length - NULL - Sign value
	mov ecx, 7	 				; divisor
	mov eax, dword [diamSum]	; move sum into eax

loopTwo:

	cdq							; convert double to quadword
	div ecx						; divide sum by 7
	push dx						; push remainder to stack
	inc r9w						; increment count digits

	cmp ax, 0					; compare answer to 0
	jne loopTwo					; loop if not 0

	sub r12w, r9w				; subtract string length by number of digits

loopThree:

	mov dword [rsi], " "		; add spaces
	inc rsi						; increment address
	...
	mov byte [rsi], r8b			; add signed value

loopFour:

	inc rsi						; increment address
	pop ax						; pop stack
	add al, 0x30				; convert int value to char
	mov byte [rsi], al			; add char to string

	dec r9w						; decrement int count
	cmp r9w, 0					; if int count is equal to 0
	jne loopFour				; loop if not equal

	inc rsi						; increment address
	mov dword [rsi], NULL		; add NULL to end of string
